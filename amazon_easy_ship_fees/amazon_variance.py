...
class AmazonReco:
    ...
    def get_state(self):
        ...
        query = f"""
                ...
                pr.height,pr.weight,c.type,c.pincode FROM sales AS s
                ...
                (s.date BETWEEN '{date_six_prior}' and '{date_now}')) GROUP BY p.OrderId
            """
        print(query)
        try:
            self.cursor.execute(query)
        ...
        for i in data:
            ...
            print("applicable weight: ", applicable_weight, shipping_zone, self.applicable_zone, sep='--')

            print("applied_weight", applied_weight, shipping_zone, self.applicable_zone, sep='--')
            ...
            if shipping_zone or not shipping_zone:
                sql = "INSERT INTO shipping_variance (channel, warehouse, order_id, item_id, applied_zone, " \
                      ...
                      "applicable_weight = %s"
                val = (
                    channel, whid, order_id, order_item_id, shipping_zone, self.applicable_zone, applied_shipping_fee,
                    applicable_shipping_fee, gap,
                    sale_status, '', order_date, applied_weight, applicable_weight, channel, whid, order_id,
...
if __name__ == '__main__':
    OBJ = AmazonReco(79030)
    OBJ.get_state()
    quit()
    base_limit = 1
    f = open('user_track_amazon.txt', 'r')
    start = int(f.read())
    print(start)
    ...
    while start <= len(user_count):
        ...
        query = f"SELECT i.UserId FROM inv_userlist AS i WHERE i.exp_date > NOW() AND i.type = 'amazon' ORDER BY i.UserId DESC LIMIT {start}, {base_limit}"
        print(query)
        ...
        for user in data:
            try:
                print(user[0])
                OBJ = AmazonReco(str(user[0]))
                OBJ.get_state()
            except TypeError as e:
                print(e)
        f_write = open('user_track_amazon.txt', 'w+')
        start = start + base_limit
        print(start)
        f_write.write(str(start))
        if start == int(len(user_count)):
            f_write.seek(0)
            f_write.truncate()
            f_write.write(str(0))
            break
