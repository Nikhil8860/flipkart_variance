...
if __name__ == '__main__':
    base_limit = 1
    f_user = open('user_track_reverse.txt', 'r')
    start = int(f_user.read())
    ...
    while start <= len(user_count):
        ...
        query = f"SELECT i.UserId FROM inv_userlist AS i WHERE i.exp_date > NOW() AND i.type = 'flipkart' LIMIT {start}, {base_limit}"
        ...
        for user in data:
            try:
                OBJ = ShippingVariance(str(user[0]))
                OBJ.get_state()
            except TypeError as e:
                print(e)
        f_write_user = open('user_track_reverse.txt', 'w+')
        start = start + base_limit
        f_write_user.write(str(start))
        if start == len(user_count):
            f_write_user.seek(0)
            f_write_user.truncate()
            f_write_user.write(str(0))
            break
        f_write_user.close()
        f_user.close()
