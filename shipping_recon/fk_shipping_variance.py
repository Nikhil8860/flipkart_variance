    ...
    def get_state(self):
        ...
        try:
            query = f"""
                    ...
                    WHERE (s.whid IS NULL s.PinCode IS NOT NULL AND s.PinCode!=0 AND s.shipmentLength IS NOT NULL 
            ...
            query_1 = f"""
                    ...
                    LEFT JOIN payments_process AS p ON p.OrderItemID=s.OrderItemID
                    LEFT JOIN products AS pr ON pr.code = s.SKUcode
                    WHERE (s.whid IS NOT NULL s.PinCode IS NOT NULL AND s.PinCode!=0 AND s.shipmentLength 
                    IS NOT NULL AND ch.postal_code IS NOT NULL AND c.type='flipkart' AND 
                    (s.date BETWEEN '{date_six_prior}' and '{date_now}')) GROUP BY p.OrderItemID
            """
